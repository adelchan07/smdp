...
class TestSimpleValueIteration(unittest.TestCase):
  def setUp(self):
    ...
    bellmanUpdate = targetCode.BellmanUpdate(stateSet, optionSpace, transition, reward, gamma)
    valueItSetUp = targetCode.ValueIteration(stateSet, optionSpace, convergence, bellmanUpdate)
    V = valueItSetUp()
    policySetUp = targetCode.GetPolicy(stateSet, optionSpace, transition, reward, gamma, V, convergence)
