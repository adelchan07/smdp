	...
	def __call__(self, stateSpace, availableOptions):
		...
		for i in range(self.numEpisodes):
			...
			while state not in self.goalStates:
				...
				sPrime = sum([self.transitionFunction(state, option, possibleSPrime) for possibleSPrime in stateSpace]) #not sure if this is the best way but this is what is done in the valueIteration code
